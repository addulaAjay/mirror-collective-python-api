...
class TestConversationModels:
    ...
    def test_conversation_title_generation(self):
        ...
        conversation = Conversation(
            conversation_id="test-conv-id", user_id="test-user-id"
        )

        title = conversation.generate_title_from_content(
            "This is a long message that should be truncated"
        )
        assert len(title) <= 60
...
class TestConversationService:
    ...
    async def test_create_conversation_success(self, conversation_service):
        ...
        mock_conversation = Conversation(
            conversation_id="test-conv-id", user_id="test-user-id", title="Test Title"
        )
        conversation_service.dynamodb_service.create_conversation = AsyncMock(
            return_value=mock_conversation
        )

        # Create conversation
        result = await conversation_service.create_conversation(
            "test-user-id", "Test Title"
        )
    ...
    async def test_get_conversation_success(self, conversation_service):
        ...
        conversation_service.dynamodb_service.get_conversation = AsyncMock(
            return_value=mock_conversation
        )

        result = await conversation_service.get_conversation(
            "test-conv-id", "test-user-id"
        )
    ...
    async def test_add_message_success(self, conversation_service):
        ...
        conversation_service.dynamodb_service.create_message = AsyncMock(
            return_value=mock_message
        )
        conversation_service.dynamodb_service.update_conversation = AsyncMock(
            return_value=mock_conversation
        )

        result = await conversation_service.add_message(
            "test-conv-id", "test-user-id", "user", "Test message"
        )
    ...
    async def test_add_message_validation_error(self, conversation_service):
        ...
        with pytest.raises(ValidationError, match="Role must be"):
            await conversation_service.add_message(
                "test-conv-id", "test-user-id", "invalid-role", "Test message"
            )
...
class TestEnhancedMirrorChatUseCase:
    ...
    def test_request_validation_success(self):
        ...
        request = EnhancedMirrorChatRequest(
            message="Hello", user_id="test-user-id", create_new_conversation=True
        )
    ...
    def test_request_validation_missing_message(self):
        ...
        request = EnhancedMirrorChatRequest(
            message="", user_id="test-user-id", create_new_conversation=True
        )
    ...
    def test_request_validation_missing_user_id(self):
        ...
        request = EnhancedMirrorChatRequest(
            message="Hello", user_id="", create_new_conversation=True
        )
    ...
    def test_request_validation_no_conversation_context(self):
        ...
        with pytest.raises(
            ValidationError, match="Either conversation_id must be provided"
        ):
            request.validate()
    ...
    async def test_execute_new_conversation(self, chat_use_case):
        ...
        chat_use_case.conversation_service.create_conversation = AsyncMock(
            return_value=mock_conversation
        )
        ...
        chat_use_case.conversation_service.get_conversation = AsyncMock(
            return_value=mock_conversation
        )
        chat_use_case.conversation_service.is_persistence_enabled = Mock(
            return_value=True
        )
        ...
        assert (
            chat_use_case.conversation_service.add_message.call_count == 2
...
class TestConversationManagementUseCase:
    ...
    async def test_get_user_conversations_success(self, management_use_case):
        ...
        management_use_case.conversation_service.get_user_conversations = AsyncMock(
    ...
    async def test_archive_conversation_success(self, management_use_case):
        ...
        management_use_case.conversation_service.archive_conversation = AsyncMock(
            return_value=True
        )
    ...
    async def test_delete_conversation_success(self, management_use_case):
        ...
        management_use_case.conversation_service.delete_conversation = AsyncMock(
            return_value=True
        )
    ...
    async def test_update_conversation_title_success(self, management_use_case):
        ...
        management_use_case.conversation_service.update_conversation_title = AsyncMock(
            return_value=True
        )

        result = await management_use_case.update_conversation_title(
