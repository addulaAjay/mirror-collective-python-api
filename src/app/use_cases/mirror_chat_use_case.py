...
class MirrorChatRequest:
    ...
    def __init__(self, message: str, conversation_history: Optional[List[ChatMessage]] = None, user_name: Optional[str] = None):
    ...
    async def execute(self, request: MirrorChatRequest) -> MirrorChatResponse:
        ...
        messages: List[ChatMessage] = [
            ...
            *request.conversation_history,  # Include previous conversation turns
            ChatMessage(role='user', content=request.message)  # Add current user message
        ]
        
        logger.debug(f"Processing mirror chat with {len(messages)} messages for user: {request.user_name or 'anonymous'}")
