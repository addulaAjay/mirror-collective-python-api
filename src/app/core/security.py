...
def map_claims_to_profile(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "createdAt": datetime.fromtimestamp(payload.get('auth_time', payload.get('iat', 0)), tz=timezone.utc).isoformat().replace('+00:00', 'Z'),
...
async def get_current_user(
    ...
    """
    is_development = os.getenv('NODE_ENV') in ['development', 'test'] or os.getenv('DEBUG', 'false').lower() == 'true'
    ...
    try:
        ...
        if is_development:
            logger.warning(f"🔧 Development: JWT decode failed, using fallback mock user: {str(e)}")
    ...
    if is_development:
        logger.info(f"🔧 Development: Decoded JWT token for user: {user_profile.get('email', 'unknown')}")
    else:
        logger.debug(f"✅ Production: User authenticated: {user_profile.get('email', 'unknown')}")
