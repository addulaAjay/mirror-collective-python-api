...
class UserService:
    ...
    async def get_or_create_user_profile(self, user_id: str, force_cognito_sync: bool = False) -> UserProfile:
        """
            ...
            if not user_profile or force_cognito_sync:
                try:
                    ...
                    if user_profile:
                        ...
                        user_profile = UserProfile.from_cognito_user(cognito_data, user_id)
                    ...
                    logger.warning(f"Failed to sync with Cognito for user {user_id}: {cognito_error}")
                    ...
                    user_profile = await self.dynamodb_service.create_user_profile(user_profile)
                    logger.info(f"Created minimal user profile due to Cognito sync failure: {user_id}")
